...
router.get('/schools/:OrganizationUUID', async (req: Request, res: Response) => {
	try {
		...
		const schools = await service.getSchools(pathSegments) as SchoolQuerySchema[];
...
// this function should be de deleted in the future
router.get('/school/:SchoolUUID/classes', async (req: Request, res: Response) => {
	try {
		const { SchoolUUID } = req.params
		const classes = await service.getClasses([SchoolUUID]) as ClassQuerySchema[]
