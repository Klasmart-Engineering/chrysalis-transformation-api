...
const processFeedback = (responses: BackendResponse[]) => {
  const isOnboard = [];
  for (const response of responses) {
    isOnboard.push(
      response.success ||
        typeof response.errors?.entityAlreadyExists === 'object'
    );
  }
...
const processErrors = (responses: BackendResponse[]) => {
  const errorMessages = [];
  for (const response of responses) {
    if (!response.success) {
      const action =
        Object.keys(requestIds)[
          Object.values(requestIds).indexOf(response.requestId.id)
        ];
      errorMessages.push(
        ...mapErrors(response.errors).map(
          (error) => `${action}: ${error.normalize('NFKC')}`
        )
      );
      messages = [];
    }
...
export function protobufToEntity(e: proto.Entity): Entity {
  switch (e) {
    ...
    case proto.Entity.ROLE:
      return Entity.ROLE;
    default:
      throw new Error('Unable to map protobuf Entity to app Entity');
  }
}
...
export const alreadyProcess = (
  ...
  if (!entities) {
    return new AlreadyProcessedResponse(
      `${pluralName} already processed`,
      [],
      []
    );
  }
  ...
  const alreadyProcessed = entities.map((entity) => {
    return { [uuidKey]: entity[uuidKey] };
  });
