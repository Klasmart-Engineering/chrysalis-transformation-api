import { FeedbackResponse, SchoolQuerySchema } from './../../../src/interfaces/clientSchemas';
...
describe('alreadyProcessed', function () {
  it('works for no data to onboard', function () {
    expect(alreadyProcess(null, Entity.SCHOOL, feedback)).to.be.jsonSchema(alreadyProcessed);
  });

  it('works for full data to onboard', function () {
    expect(alreadyProcess(schools, Entity.SCHOOL, feedback)).to.be.jsonSchema(alreadyProcessed);
  });
});
